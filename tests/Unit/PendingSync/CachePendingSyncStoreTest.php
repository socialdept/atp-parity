...
class CachePendingSyncStoreTest extends TestCase
{
    ...
    public function test_remove_cleans_up_indexes(): void
    {
        $pendingSync = $this->createPendingSync('id1', 'did:plc:test');
        $this->store->store($pendingSync);

        $this->assertSame(1, $this->store->countForDid('did:plc:test'));

        $this->store->remove('id1');

        $this->assertSame(0, $this->store->countForDid('did:plc:test'));
    }
    ...
    public function test_storing_replaces_existing_by_model(): void
    {
        // Store first sync for model
        $sync1 = $this->createPendingSync('id1', 'did:plc:user1', 'App\\Post', 42);
        $this->store->store($sync1);

        $this->assertSame(1, $this->store->countForDid('did:plc:user1'));

        // Store second sync for same model (should NOT replace - that's manager's job)
        $sync2 = $this->createPendingSync('id2', 'did:plc:user1', 'App\\Post', 42);
        $this->store->store($sync2);

        // Both should exist (deduplication is handled by manager, not store)
        $this->assertSame(2, $this->store->countForDid('did:plc:user1'));
    }
